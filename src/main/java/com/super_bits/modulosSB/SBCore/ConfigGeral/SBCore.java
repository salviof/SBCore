...
public class SBCore {
        ...
        try {
            ...
            if (!ignorarConfigurcoesDePermissao) {
                if (!ignorarConfigurcoesDeAcoes) {
                    ...
                    CarameloCode.getServicoMensagemFireForget().enviarMsgAlertaAoUsuario("ATENÇÃO, SERVIÇO DE PERMIÇÃO ATIVADO SEM O SERVIÇO DE AÇÕES DO SISTEMA, IMPOSSÍVEL VINCULAR USUÁRIOS AS AÇÕES, ESSE MODELO DE APLICAÇÃO SE APLICA APENAS PARA WEB-SERVICES DE SERVIÇOS ESPECÍFICOS QUE NÃO RODAM AS REGRAS DE NEGÓCIO DO SISTEMA");
    ...
    /**
     *
     * Trata o erro de acordo com o contexto de execução o tipoTratamentoErro
     * determina como o erro deve ser tratado
     *
     * @param pErro
     */
    public static void RelatarErro(ErroCRCCoreFW pErro) {
        RelatarErro(pErro.getTipoTratamentoErro(), pErro.getMessage(), pErro);
    }
